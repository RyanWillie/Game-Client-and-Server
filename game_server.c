...
void playNumbers(int numPlayers, int sv[2], int sv2[2]);
void GameOver(int numPlayers, int winner, int sv, int sv2);
enum{CHILD, PARENT};

// Function designed for chat between client and server.
void func(int sockfd, int playerNum, int sv, int sv2)
{
	char buff[MAX];
    char msg[MAX];
	char sum[10];
...
int main(int argc, char *argv[])
{
    ...
    for(int i=0; i < maxPlayers; i++){
        ...
        if(fork() == 0){
            ...
            func(connfd, i, sv[CHILD], sv2[CHILD]);
